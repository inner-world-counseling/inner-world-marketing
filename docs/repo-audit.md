...
- [ ] **Bug: the hero preload `imagesrcset` descriptor overstates the image width.** On `/`, `/counseling-for-anxiety/` and `/emdr/` the preload declares `/images/hero-banner-leaves.webp 1600w`, but the file is **1000 px wide** (1000 × 1778). A `w` descriptor that lies about the real width can make the browser choose the wrong candidate and defeats the purpose of the responsive set. Found 2026-07-30 while reading intrinsic dimensions for the width/height fix; not changed because it was outside that task's scope. The correct descriptor is `1000w`.
