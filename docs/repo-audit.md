...
- [ ] **Schema: `#organization` carries 4 distinct shapes across 28 pages — same drift class as the Person node was.** Every page defines `MedicalBusiness` with `@id` `https://deirdrekuvaas.com/#organization`, but the node differs between pages, so a consumer merging by `@id` gets conflicting data. Measured 2026-07-29 **after** the Person normalization removed `hasCredential` from this node (that axis is already gone). Three axes remain:
  ...
  Shape counts: 23 pages / 3 state pages / `/` / `/ohio-telehealth-therapy/`. Deliberately **not** folded into the Person normalization (PR #60) so that diff stays reviewable while it carries the LADC hold and the unverified LinkedIn URL. Next pass.
