...
- [ ] **The LinkedIn `sameAs` target is still not linked from any markup.** Every other `Person.sameAs` target — NPI registry, Psychology Today, EMDRIA, GoodTherapy — is also a real link in the footer of all 29 pages. LinkedIn is asserted in structured data only. Now that the URL is confirmed correct, adding it to the footer social row would make the `sameAs` set consistent with what the site actually links. Not done: it is a visible-markup change to the footer, which is duplicated across 29 pages with known drift, so it needs its own scoped pass rather than riding along with a JSON-LD edit.
