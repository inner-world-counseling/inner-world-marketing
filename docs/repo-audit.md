...
- [ ] **Copy: three homepage statements name ND, MN and UT but omit Ohio — not covered by the market-targeting decision above.** Flagged 2026-07-29, **not** changed. These sit awkwardly between the two categories: they are not ND/MN-focused targeting copy, because they already break that focus by naming Utah, yet they omit Ohio, which is an equally active licensure state. Two of the three are availability claims rather than positioning.
  ...
  Note the meta descriptions are what search and social previews display, so they reach users who never load the page. Resolution is a one-line decision: either these are targeting copy that happens to include UT and should be left alone, or they are availability claims and Ohio should be added. Note also that the `twitter:description` has a lowercase sentence fragment ("addiction. serving ND, MN & UT.") independent of the state question.
