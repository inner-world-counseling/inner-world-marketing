...
- [ ] **Copy: the ND/MN/UT three-state form appears 47 times across 14 other pages, almost entirely in `<title>` and `<meta>`. NOT fixed — needs a decision pass.** Found 2026-07-29 by a site-wide scan while fixing the homepage; only `index.html` was in scope. These matter more than body copy because `<title>` and `<meta description>` are what search results and social cards display, so they reach people who never load the page. Most are phrased as **availability**, which by the rule recorded above means they should name all four states: *"Telehealth sessions … in ND, MN, & UT"*, *"serving ND, MN & UT"*, *"Telehealth in ND, MN, & UT"*, *"from anywhere in ND, MN, & UT"*, *"Serving adults across ND, MN, & UT"*, *"Online trauma therapy in ND, MN, and UT"*.
